...
class DDQN:
    def __init__(self, env):
        ...
        self.Replay_Size = 1000000  # 定义经验池大小
        self.Batch_Size = 32  # 定义mini_batch大小
    ...
    def train_Q_network(self, k=0):  # 定义训练
        ...
        for i in range(0, self.Batch_Size):
            done = minibatch[i][2 * self.current_Q_net.state_dim + self.current_Q_net.action_dim + 1]   # 取出 minibatch 中每一行的第12个数据，即取出是否到达终止的标识
            if done:
    ...
    def update_target_params(self):  # 更新目标网络参数
        ...
        self.target_Q_net.load_state_dict(torch.load('net_params.pkl'))  # 上穿当前网络参数并赋给目标网络
